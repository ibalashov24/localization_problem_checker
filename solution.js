...
var side_half_corridor_width = 33; // in infrared sensor points
var forward_half_corridor_width = 33; // in sonic sensor points
...
// Calculates coefficient for proportional 
// controller which reflect IR sensors indication
var calc_alignment = function()
{
    ...
    if (left_distance != 0 && left_distance < right_distance 
		&& left_distance < 2 * side_half_corridor_width)
    {
        alignement = -side_half_corridor_width + left_distance;
    }
    else if (right_distance != 0 && right_distance < left_distance 
		&& right_distance < 2 * side_half_corridor_width)
    {
...
// Reads map from input array
var read_map = function()
{
	var map_height = input_map.length;
	var map_width = input_map[0].length;
	for (var i = 0; i < map_height; ++i)
	{
		if (input_map[i].length != map_width)
		{
			throw new Error("Map array must be rectangular!!!");
		}
	}
	
	result = new Array(input_map.length)
	for (var i = 0; i < input_map.length; ++i)
	{
		result[i] = new Array(input_map[0].length);
		
		for (var j = 0; j < input_map[0].length; ++j)
		{
